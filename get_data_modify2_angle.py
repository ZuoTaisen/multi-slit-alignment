...
files = glob.glob('ave_angle_blueperple*.dat')
files = sorted(files,key = lambda x :float(x[24:26]))
...
def get_rid_of_bad_points(input_array,critiarial):
    ave_poses = input_array
    ave_poses2 = []
    average = np.average(ave_poses)
    ave_poses2.append(average)
    for i in range(1,len(ave_poses)):
        if ave_poses[i] > ave_poses[i-1]-critiarial and ave_poses[i] < ave_poses[i-1]+critiarial:
            ave_poses2.append(ave_poses[i]) 
        else:
            ave_poses2.append(ave_poses2[-1]) 
    return ave_poses2

for i,file in enumerate(files[:]):
    pix = pd.read_csv(file,sep = '\s+',usecols = [0,1,2,3,4,5,6,7],names = ['col1','col2','col3','col4','col5','col6','col7','col8'])
    x = np.arange(len(pix['col1']))
    sum = np.zeros(len(pix['col1']))
    for j in range(1,9):
        new_array = get_rid_of_bad_points(pix['col'+str(j)],0.4)
        sum += new_array
    y = sum/8
    ave_y = np.average(y)
    ...
    plt.ylabel('Angles (deg.)',size = 20)
plt.savefig('Angle_variation'+str(col)+'.png',format = 'png',dpi = 600, transparent = True,bbox_inches = 'tight')
...
ax.set_ylabel('Average angle (deg.)',size = 20)
ax.tick_params(axis = 'both',direction = 'in',labelsize = 15,width = 2)
plt.savefig('average_angle'+str(col)+'.png',format = 'png',dpi = 600, transparent = True,bbox_inches = 'tight')
with open('average_angle'+str(col)+'.dat','w') as f:
