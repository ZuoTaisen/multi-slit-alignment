...
def save_pos_angle(ave_poses,ave_angles,file_name):
    xx = np.arange(len(ave_poses))
    with open('ave_pos_blueperple_' + file_name + '.dat','w+') as f:
        save_to_file(xx,ave_poses,f)
    with open('ave_angle_blueperple_' + file_name + '.dat','w+') as f:
...
###############input parameters################
directory = r'D:\work\workRelated\hezhengqiang\blueperple-modify2\test'
slit_num = 1
###############input parameters################

directory = pathlib.Path(directory)
first_time, first_file_path = max((f.stat().st_mtime, f) for f in directory.iterdir())
ave_poses = []
ave_angles = []
if slit_num >= 4:
    peak_width = 40
for i in range(100):    
    ...
    if file_path != first_file_path and str(file_path).split('.')[-1] == 'bmp':
        print(datetime.fromtimestamp(time), file_path)
        peak_poses, ave_pos, ave_angle = get_ave_pos(str(file_path),peak_width)
        ave_poses.append(ave_pos)
        ave_angles.append(ave_angle)
        #print('Ave postion is:' + ' ave_pos)
        #print('Ave angle is :' + ' ave_angle)
    else:
        Time.sleep(1)
    first_time = time
    first_file_path = file_path
    save_pos_angle(ave_poses,ave_angles,'slit' + str(slit_num))
